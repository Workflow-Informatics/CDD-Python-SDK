...
- [CDD-Python-SDK](#cdd-python-sdk)
	...
	- [Protocols](#protocols)
		- [Return a list of accessible protocols for the given vault](#return-a-list-of-accessible-protocols-for-the-given-vault)
	...
	- [Batch Move Jobs](#batch-move-jobs)
		- [This endpoint requires the user to be a vault admin](#this-endpoint-requires-the-user-to-be-a-vault-admin)
		...
		- [Cancel a single batch move job in the queue](#cancel-a-single-batch-move-job-in-the-queue)
			- [NOTE: Once a job has started it cannot be deleted. Also, if you are moving the highest batch of a molecule, the batch number it previously occupied will be reused by the next batch of the original molecule](#note-once-a-job-has-started-it-cannot-be-deleted-also-if-you-are-moving-the-highest-batch-of-a-molecule-the-batch-number-it-previously-occupied-will-be-reused-by-the-next-batch-of-the-original-molecule)
...
```python
from cdd_python_sdk.VaultClient import VaultClient as cdd
```

Confirm your [User Permissions](https://support.collaborativedrug.com/hc/en-us/articles/214359023-Vault-User-Roles), then instantiate a VaultClient to work with your data:
...
vault = cdd(vault_id, api_key)

# confirm your VaultClient:
print(str(vault.vaultNum) + ' / ' + vault.apiKey +' / ' + vault.URL)
```
...
protocols_json = vault.getProtocols(asDataFrame=False) # <--- all projects, JSON response

filtered_protocols = vault.getProtocols(projects = projects_dataframe.at[0, 'id']) #<--- filter projects
```

A full list of valid parameters can be returned by passing  `help=True`
